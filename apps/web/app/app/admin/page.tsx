...
import { AdminActionForm, AdminSubmitButton } from "./admin-form-client";

const ADMIN_SECTIONS = ["overview", "organizations", "facilities", "users"] as const;
type AdminSection = (typeof ADMIN_SECTIONS)[number];

function normalizeSection(value: string | undefined): AdminSection {
  if (value && ADMIN_SECTIONS.includes(value as AdminSection)) return value as AdminSection;
  return "overview";
}

function adminSectionHref(section: AdminSection): string {
  return `/app/admin?section=${section}`;
}
    ...
    <>
      <section className="rounded-[24px] border border-slate-200/70 bg-white/78 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.10)] backdrop-blur sm:p-6 md:p-8 xl:p-10">
        <div className="space-y-6">
          ...
          <nav aria-label="Admin sections" className="flex flex-wrap gap-2 border-b border-slate-200/80 pb-4">
          ...
          {currentSection === "overview" && (
            <>
              ...
              <p className="text-sm leading-6 text-slate-600">
                Use the tabs above to manage organizations, facilities, and user access.
              </p>
            </>
          )}
          ...
          {currentSection === "organizations" && (
            <section className="grid gap-5 sm:gap-6">
              <div className="rounded-[24px] border border-slate-200/70 bg-white/78 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur sm:p-6 md:p-7">
                ...
                <AdminActionForm action={createOrganizationAction} className="mt-6 space-y-4">
                  ...
                  <AdminSubmitButton loadingLabel="Creating...">
                    Create organization
                  </AdminSubmitButton>
                </AdminActionForm>
              </div>
          ...
          {currentSection === "facilities" && (
            <section className="grid gap-5 sm:gap-6">
              <div className="rounded-[24px] border border-slate-200/70 bg-white/78 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur sm:p-6 md:p-7">
                ...
                <AdminActionForm action={createFacilityAction} className="mt-6 grid gap-4 md:grid-cols-2">
                  ...
                  <AdminSubmitButton className="md:col-span-2" loadingLabel="Creating...">
                    Create facility
                  </AdminSubmitButton>
                </AdminActionForm>
              </div>
          ...
          {currentSection === "users" && (
            <section className="space-y-6">
              <div className="rounded-[30px] border border-slate-200/70 bg-white/78 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.08)] backdrop-blur sm:p-6 md:p-7">
                ...
                <AdminActionForm action={inviteMembershipAction} className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  <label className="block space-y-2">
                  ...
                  <div className="flex items-end">
                    <AdminSubmitButton className="w-full" loadingLabel="Inviting...">
                      Invite and assign
                    </AdminSubmitButton>
                  </div>
                </AdminActionForm>
